...
export default function Home(){
 ...
 let soundBank:any=null,soundscape:any=null;const earFacing=new THREE.Vector3(),SCRAMBLE_EAR={x:6.54,z:1.99};
 ...
 const enterPlayer=()=>{const ctx=lifeEntry.hooks.current?.network?.ctx;
  ...
  playerMode=true;combatDeathReported=false;controls.enabled=false;player.attach(canvas,{onExit:()=>exitPlayer(),onDrive:()=>toggleDrive(),onAttack:()=>attack(),onHorn:()=>{if(driving&&playerCar)soundscape?.horn(playerCar.state.x,playerCar.state.z);}});setPlayerHit(null);setMode('player');
