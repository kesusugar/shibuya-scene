...
export default function Home(){
 ...
 let soundBank:any=null,soundscape:any=null,police:any=null;const earFacing=new THREE.Vector3(),SCRAMBLE_EAR={x:6.54,z:1.99};
 ...
 const enterPlayer=()=>{const ctx=lifeEntry.hooks.current?.network?.ctx;
  ...
  if(!police)police=createPoliceDirector({getAudioContext:()=>playerAudio?.context??null,getAudioBus:()=>soundBank?.bus??null});
  void soundBank.load();
  ...
  if(touchEnabled&&!touchPad)touchPad=createTouchControls({
   onAxes:(axes:any)=>player?.setTouch(axes),onAttack:()=>attack(),
  ...
  playerMode=true;combatDeathReported=false;controls.enabled=false;player.attach(canvas,{onExit:()=>exitPlayer(),onDrive:()=>toggleDrive(),onAttack:()=>attack(),onHorn:()=>{if(driving&&playerCar&&!police?.toggleSiren(playerCar))soundscape?.horn(playerCar.state.x,playerCar.state.z);}});setPlayerHit(null);setMode('player');
 ...
 if(playerMode&&player){const crowdSim=lifeEntry.hooks.current?.sim;const queue=crowdSim?.splashes;if(queue?.length){for(const q of queue)blood?.splash(q.x,q.y,q.z,q.dx,q.dz,q.scale,q.life);queue.length=0;}
  ...
  if(police){const w=police.frame(dt,{player:player.state,car:playerCar,driving,melee:melee.snapshot(),traffic:trafficEntry.hooks.current?.sim,crowd:crowdSim,listener:{x:view.position.x,z:view.position.z,vx:0,vz:0}});playUI?.setWanted(w,dt);(window as any).__SHIBUYA_POLICE__=police;}
 }
