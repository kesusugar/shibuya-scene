...
export function buildSignModel(data,{tier='medium',generic=buildBuildingModel(data),ground=buildGroundModel(data),heroes,core=buildStationModel(data,{ground,generic})}={}){
 ...
 for(const b of generic.buildings){const region=regionAt(b.centroid),density=REGIONS[region].density*q.factor;if(b.archetype==='balcony'&&region==='peripheral')continue;
  ...
  const slot=Math.max(1,Math.floor((e.length-.5)/5));const rows=Math.min(region==='centerGai'?4:3,Math.floor((b.height-1.5)/3.5));
  for(let row=0;row<rows;row++)for(let col=0;col<slot;col++){const roll=rng(),kindRoll=rng(),variant=Math.floor(rng()*32);if(roll>density)continue;const w=Math.min(5,e.length/slot-.45),height=row===0?1:1.55,y=b.base+2.5+row*3.5;let category=row===0?(col%2?'entrance':'box'):kindRoll<.35?'directory':kindRoll<.65?'flush':kindRoll<.85?'billboard':'screen';submit(makePlacement(host,e,{id:b.key+':'+row+':'+col,category,along:(col+.5)*e.length/slot,y,width:category==='directory'?Math.min(w,1):w,height:category==='directory'?2.6:height,region,variant}));}
