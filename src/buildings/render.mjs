 ...
 for(const b of model.buildings){
  ...
  for(let edge=0;edge<b.polygon.outer.length;edge++){
   ...
   for(let floor=0;floor<floorCount;floor++){
    ...
    const isShop=b.archetype==='shop'&&floor===0&&front;
    ...
    for(let col=0;col<cols&&windowCount<C.maxWindowsPerBuilding;col++){
     ...
     if(width<=.2)continue;const bucket=isShop?'shopWindows':curtain?'curtainWindows':'windows';const colors=curtain?[0x607f90,0x7897a4,0x455e70]:isShop?[0x475b60,0x627b7d]:[0x3e5666,0x68818a,0x849295];const color=colors[Math.floor(rng()*colors.length)];const p=point(along,y);
