...
export function paintCentralRetail(ctx,w,h){
 ...
 for(const bay of [2,7,10]){
  const bw=w/12,x=bay*bw;
...
export function createCentralPolish(heroes){
 ...
 for(const hero of heroes){if(!CENTRAL.has(hero.key))continue;const candidates=edges(hero.footprint).filter(e=>e.length>4&&e.normal[0]*hero.primaryFacade.direction[0]+e.normal[1]*hero.primaryFacade.direction[1]>.2);for(const e of candidates){fronts++;const isQfront=hero.key==='qfront';if(!isQfront){const g=new PlaneGeometry(e.length-.28,7.25);g.rotateY(e.heading);g.translate(...facadePoint(e,e.length/2,3.78,.31));retailPieces.push(g);}const bays=Math.max(2,Math.round(e.length/2.6));for(let i=0;!isQfront&&i<=bays;i++){const along=.18+(e.length-.36)*i/bays;framePieces.push(boxGeometry(.11,7.35,.16,facadePoint(e,along,3.78,.42),e.heading));}for(const y of [.48,3.55,7.18])framePieces.push(boxGeometry(e.length-.2,.12,.17,facadePoint(e,e.length/2,y,.42),e.heading));canopyPieces.push(boxGeometry(e.length-.42,.2,1.05,facadePoint(e,e.length/2,3.25,.78),e.heading));if(isQfront){const g=new PlaneGeometry(Math.max(2,e.length-.5),.92);g.rotateY(e.heading);g.translate(...facadePoint(e,e.length/2,7.08,.48));marqueePieces.push(g);}}}
