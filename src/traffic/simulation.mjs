...
export class TrafficSimulation{
 ...
 for(const v of this.pool){if(!v.active||v.parked||staged>=30)continue;
  for(let attempt=0;attempt<this.heroLanes.length;attempt++){
   ...
   rows.set(lane.id,progress-half);staged++;this.rebuildGrid();break;
  }
