...
export const FIDELITY={dpr:1.5,shadow:2048,aoScale:.5,radius:2.5,thickness:1.5,samples:12,blend:.85,denoiseRings:2,denoiseSamples:12};
...
class SoftBloomPass extends Pass{
 constructor(){super();this.strength=.18;this.soft=new WebGLRenderTarget(1,1,{type:HalfFloatType,depthBuffer:false});this.extract=new ShaderMaterial({depthTest:false,depthWrite:false,toneMapped:false,uniforms:{source:{value:null},stepUV:{value:new Vector2()}},vertexShader:vertex,fragmentShader:`uniform sampler2D source;uniform vec2 stepUV;varying vec2 vUv;void main(){vec3 sum=vec3(0.);for(int x=-1;x<=1;x++)for(int y=-1;y<=1;y++){vec3 c=texture2D(source,vUv+vec2(float(x),float(y))*stepUV*1.5).rgb;float b=max(c.r,max(c.g,c.b));sum+=c*max(b-1.1,0.)/max(b,.001);}gl_FragColor=vec4(sum/9.,1.);}`});this.combine=new ShaderMaterial({depthTest:false,depthWrite:false,toneMapped:false,uniforms:{source:{value:null},glow:{value:this.soft.texture},strength:{value:.18}},vertexShader:vertex,fragmentShader:'uniform sampler2D source;uniform sampler2D glow;uniform float strength;varying vec2 vUv;void main(){gl_FragColor=vec4(texture2D(source,vUv).rgb+texture2D(glow,vUv).rgb*strength,1.);}'});this.quad=new FullScreenQuad();}
 ...
 render(r,write,read){this.extract.uniforms.source.value=read.texture;this.quad.material=this.extract;r.setRenderTarget(this.soft);this.quad.render(r);this.combine.uniforms.source.value=read.texture;this.combine.uniforms.strength.value=this.strength;this.quad.material=this.combine;r.setRenderTarget(this.renderToScreen?null:write);this.quad.render(r);}
...
export function createFidelityPipeline(renderer,scene,camera){
 ...
 return {composer,ao,bloom,grade,smaa,passes,render(night,wetEnabled){if(disposed)return;renderer.getDrawingBufferSize(size);if(size.x!==w||size.y!==h){w=size.x;h=size.y;composer.setSize(w,h);}bloom.enabled=night&&wetEnabled;bloom.strength=.18;grade.uniforms.night.value=night?1:0;const auto=renderer.info.autoReset,target=renderer.getRenderTarget();renderer.info.autoReset=false;if(auto)renderer.info.reset();try{composer.render();}finally{renderer.info.autoReset=auto;renderer.setRenderTarget(target);}},dispose(){if(disposed)return;disposed=true;passes.forEach(p=>p.dispose?.());composer.dispose();}};
}
