...
{id:'scramble',label:'CAM-02 Scramble High',position:[58,62,67],target:[-10,8,-15]},
{id:'street',label:'CAM-03 Scramble Street',position:[18,1.55,25],target:[-15,8,-32]},
...
export function parseConfig(search='') {const q=new URLSearchParams(search),qa=q.get('qa')==='1',requestedCamera=q.get('camera'),camera=requestedCamera==='scramble-street'?'street':requestedCamera;const list=k=>q.has(k)?q.get(k).split(',').map(s=>s.trim()).filter(Boolean):null;return {qa,tier:qa?'high':Object.hasOwn(PROFILES,q.get('tier'))?q.get('tier'):'medium',camera:CAMERAS.some(c=>c.id===camera)?camera:'overview',time:q.get('time')==='night'?'night':'day',explicitTime:q.has('time'),only:list('only'),skip:list('skip')??[],debug:q.get('debug')==='1'};}
