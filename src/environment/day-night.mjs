...
export function installNightEmission(material,mode){
 const original=material.onBeforeCompile,key=material.customProgramCacheKey,uniform={value:0};
 material.onBeforeCompile=shader=>{original.call(material,shader);shader.uniforms.s12Night=uniform;
  let vertex='',fragment='uniform float s12Night;\n',body='';
  if(mode==='window'){vertex='varying float s12Window;\n';fragment+='varying float s12Window;\n';body=`s12Window=0.0;
#ifdef USE_INSTANCING
float seed=mod(abs(dot(instanceMatrix[3].xyz,vec3(12.9898,78.233,37.719))),7.0);
s12Window=seed<3.0 ? 0.35+seed*0.1 : 0.0;
#endif`;
...
if(abs(cab)>0.5)s12Train=cab>0.0?vec3(1.0,0.86,0.62):vec3(0.75,0.025,0.015);`;
   shader.fragmentShader=shader.fragmentShader.replace('#include <emissivemap_fragment>','#include <emissivemap_fragment>\ntotalEmissiveRadiance += s12Train*s12Night;');
 ...
 return {uniform,restore(){uniform.value=0;material.onBeforeCompile=original;material.customProgramCacheKey=key;material.needsUpdate=true;}};
}
export class DayNightSystem{
 constructor(scene,renderer,time){this.scene=scene;this.renderer=renderer;this.time=time;this.roots=new Map();this.active=false;this.savedBackground=scene.background;this.savedExposure=renderer?.toneMappingExposure??1;this.background=new Color();this.root=new Group();this.root.name='s12-environment';this.fill=new HemisphereLight(0xeaf2ff,0x38435b,2);this.key=new DirectionalLight(0xfff6e9,2.2);this.key.position.set(-80,180,90);this.root.add(this.fill,this.key);this.off=time.subscribe(()=>this.apply());}
 ...
 register(root){if(this.roots.has(root))return;const materials=new Map(),lights=[];
  root.traverse(o=>{if(o.isLight)lights.push({light:o,intensity:o.intensity});if(!o.isMesh)return;for(const m of Array.isArray(o.material)?o.material:[o.material]){if(!m?.isMeshStandardMaterial)continue;let mode=null,night=null;
   ...
   if(/^signs-(print|led|heroScreen)$/.test(o.name))night=1.2;
   ...
   if(!mode&&night===null)continue;if(materials.has(m))continue;
   materials.set(m,{material:m,intensity:m.emissiveIntensity,night,emission:mode?installNightEmission(m,mode):null});
  }});
 ...
 unregister(root){const r=this.roots.get(root);if(!r)return;root.removeEventListener('removed',r.removed);for(const b of r.materials.values()){b.material.emissiveIntensity=b.intensity;b.emission?.restore();}for(const b of r.lights)b.light.intensity=b.intensity;this.roots.delete(root);this.apply();}
 apply(){const night=this.active&&this.time.isDark(),settings=DAY_NIGHT[this.time.value];let hemis=0,suns=0;
  for(const r of this.roots.values()){for(const b of r.lights){b.light.intensity=night?0:b.intensity;if(b.light.isHemisphereLight)hemis++;if(b.light.isDirectionalLight)suns++;}for(const b of r.materials.values()){b.material.emissiveIntensity=night&&b.night!==null?b.night:b.intensity;if(b.emission)b.emission.uniform.value=night?1:0;}}
