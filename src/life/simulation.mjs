...
export class CrowdSimulation{
 constructor(network,{traffic=null,tier='medium',seed='shibuya-s10'}={}){
 ...
 blocked(x,z,p,r=RADIUS*2+.06,includeReservations=true){if(includeReservations&&!p?.crossing)for(const [id,owners] of this.exits){const e=this.network.edges[p?.edge];if(owners.has(p?.id)||e?.crossingId&&e.to===id)continue;const n=this.network.nodes[id];const distance=Math.hypot(x-n.x,z-n.z);if(distance<.85&&(!p||distance<=Math.hypot(p.x-n.x,p.z-n.z)))return true;}const ix=Math.floor(x/2),iz=Math.floor(z/2);for(let i=ix-1;i<=ix+1;i++)for(let j=iz-1;j<=iz+1;j++)for(const q of this.grid.get(i+','+j)??[]){if(q===p||!q.active)continue;this.stats.neighborChecks++;if((q.x-x)**2+(q.z-z)**2<r*r)return true;}return false;}
 ...
 setTier(tier){if(!QUALITY[tier])throw Error('Unknown crowd tier');if(tier===this.tier)return;this.tier=tier;for(const p of this.pool){if(p.active&&!p.crossing)this.despawn(p,'profile');else if(p.active){p.group=-1;p.leader=-1;p.mode='ambient';}}this.groups.length=0;this.refill(true);}
 ...
 refill(initial=false){const q=QUALITY[this.tier];let count=this.pool.filter(p=>p.active).length;if(initial&&count===0){for(let i=0;i<q.idle;i++)if(this.spawn('idle',i%3?'hachiko':'center-gai'))count++;for(let i=0;i<q.milling;i++)if(this.spawn('milling','hachiko'))count++;
  ...
  const budget=initial?q.total:12;for(let i=0;i<budget&&count<q.total;i++){const region=i%4===0?'center-gai':i%4===1?'hachiko':null;const idle=this.pool.filter(p=>p.active&&p.mode==='idle').length,milling=this.pool.filter(p=>p.active&&p.mode==='milling').length;const mode=idle<q.idle?'idle':milling<q.milling?'milling':'ambient';if(this.spawn(mode,mode==='ambient'?region:'hachiko'))count++;}
 }
 ...
 move(p,dt){if(!p.active)return;const n=this.network,oldCell=this.cell(p.x,p.z);p.age+=dt;p.animationTime+=dt;
 ...
 step(dt){this.time+=dt;this.lodClock+=dt;this.refillClock+=dt;this.rebuild();if(this.lodClock>=1){this.lodClock=0;for(const p of this.pool)if(p.active){const d=Math.hypot(p.x-this.camera.x,p.z-this.camera.z);p.lod=d<65?'near':d<140?'mid':'far';}}
  ...
  const start=Math.floor(this.time*30)%this.pool.length;for(let j=0;j<this.pool.length;j++){const p=this.pool[(start+j)%this.pool.length];if(!p.active)continue;p.elapsed+=dt;const interval=p.crossing?1/30:p.mode==='idle'?.5:p.lod==='near'?1/30:p.lod==='mid'?1/15:.2;if(p.elapsed+1e-8<interval){this.stats.throttled++;continue;}const elapsed=p.elapsed;p.elapsed=0;this.move(p,elapsed);}
