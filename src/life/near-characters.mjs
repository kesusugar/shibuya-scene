...
export function createNearCharacters(tier='high'){
 ...
 let asset=null,disposed=false,trianglesPerRig=0;
  ...
  update(people,focus,dt,clock=0){
   ...
   if(slots.length<candidates.length){
    if(!asset){asset=bakedAsset();palette.push(...new Set(Object.values(ARCHETYPES).flatMap(a=>a.colors)));
     asset.template.traverse(o=>{if(o.isMesh)trianglesPerRig+=(o.geometry.index?.count??o.geometry.attributes.position.count)/3;});}
    const figure=createPlayerFigure(asset);root.add(figure.root);slots.push({figure,id:null,elapsed:0});}
   const wanted=new Set(candidates.map(c=>c.p.id));
   for(const s of slots)if(!wanted.has(s.id)){s.id=null;s.figure.hide();}
   selected.clear();
   for(const {p} of candidates){let slot=slots.find(s=>s.id===p.id);if(!slot){slot=slots.find(s=>s.id===null);if(!slot)continue;slot.id=p.id;slot.figure.reset();slot.figure.recolour({top:palette[Math.abs(p.id)%palette.length]});slot.figure.setHeight(ARCHETYPES[p.archetype]?.height??1.76);slot.elapsed=.1;}
