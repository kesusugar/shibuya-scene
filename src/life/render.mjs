import {Group,BoxGeometry,SphereGeometry,ConeGeometry,InstancedMesh,MeshStandardMaterial,Object3D,Color,DynamicDrawUsage,BufferGeometry,Float32BufferAttribute,LineSegments,LineBasicMaterial} from 'three';
...
export function buildCrowd(data,options={}){
 ...
 const material=new MeshStandardMaterial({color:0xffffff,roughness:.88});
 const geometry={torso:new BoxGeometry(1,1,1),head:new SphereGeometry(1,7,5),hair:new SphereGeometry(1,7,4),arms:new BoxGeometry(1,1,1),legs:new BoxGeometry(1,1,1),bag:new BoxGeometry(1,1,1),umbrella:new ConeGeometry(1,.35,8),hat:new BoxGeometry(1,1,1)};
 const meshes={};for(const [key,g] of Object.entries(geometry)){const m=new InstancedMesh(g,material,POOL_SIZE*(['arms','legs'].includes(key)?2:1));m.instanceMatrix.setUsage(DynamicDrawUsage);m.frustumCulled=false;m.name='crowd-'+key;root.add(m);meshes[key]=m;}
 const obj=new Object3D(),color=new Color(),counts={},stats={geometries:8,materials:1,textures:0,batches:8,triangles:0,debugBatches:0};let disposed=false,reportClock=0;
 ...
 function sync(dt=0){for(const k of Object.keys(meshes))counts[k]=0;for(const p of sim.pool){if(!p.active)continue;const def=ARCHETYPES[p.archetype],h=def.height*(.96+(p.id%5)*.02),w=def.width,walk=p.speed>.05,phase=p.animationTime*(walk?7:1)+p.phase,fidelity=p.lod==='near'?1:p.lod==='mid'?.65:.15,swing=walk?Math.sin(phase)*.38*fidelity:Math.sin(phase)*.025,bob=walk?Math.abs(Math.cos(phase))*.024*fidelity:Math.sin(phase)*.008;
   ...
   part('head',p,0,h*.87+bob,0,h*.075,h*.087,h*.074,skin);
   part('hair',p,0,h*.92+bob,-.015,h*.079,h*.051,h*.078,def.hood?shirt:hair);
 ...
 sync();return {root,network,sim,stats,meshes,update(dt,camera){if(disposed)return;if(camera)sim.setCamera(camera.x,camera.z);sim.update(dt);sync(dt);},setTier(t){sim.setTier(t);sync();},dispose(){if(disposed)return;disposed=true;sim.dispose();for(const m of Object.values(meshes))m.dispose();for(const g of Object.values(geometry))g.dispose();material.dispose();debug?.geometry.dispose();debug?.material.dispose();root.removeFromParent();root.clear();}};
}
