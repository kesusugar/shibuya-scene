...
export class ScrambleChoreography {
 constructor(sim){this.sim=sim;this.occupied=new Set();this.cursor=0;}
 slot(node,serial){const n=this.sim.network;
  for(let i=0;i<360;i++){const angle=(serial*2.399963+i*1.618),radius=.3+(i%60)*.095,x=node.x+Math.cos(angle)*radius,z=node.z+Math.sin(angle)*radius,key=Math.round(x/.32)+','+Math.round(z/.32);
   if(this.occupied.has(key)||!n.ctx.safe(x,z,.29)||!n.segmentSafe({x,z},node,false)||this.sim.vehicleOverlap(x,z,.35))continue;
   this.occupied.add(key);return [x,z];
  }return null;
 }
 refill(initial){const s=this.sim,q=QUALITY[s.tier],target=Math.round(q.total*.85);let active=s.pool.filter(p=>p.active),cast=active.filter(p=>p.choreographed).length;
  if(!active.length){this.occupied.clear();this.cursor=0;}
  for(let attempts=0;cast<target&&active.length<q.total&&attempts<q.total*2;attempts++){
   const p=s.pool.find(p=>!p.active),e=s.crossCandidates[this.cursor++%s.crossCandidates.length];if(!p||!e)break;
   const reverse=s.crossCandidates.find(r=>r.crossingId===e.crossingId&&r.direction===-e.direction);if(!reverse)continue;
   const a=this.slot(s.network.nodes[e.from],p.id),b=this.slot(s.network.nodes[e.to],p.id+500);if(!a||!b)continue;
   ...
   s.insert(p);s.stats.spawned++;active.push(p);cast++;
  }
 ...
 move(p,dt){const s=this.sim,t=p.track,e=p.edge>=0?s.network.edges[p.edge]:t.e;p.animationTime+=dt;p.age+=dt;p.speed=0;
  if(!p.crossing){if(t.cooldown>0){t.cooldown-=dt;p.state='exiting';return;}p.state='waiting';const cycle=Math.floor((s.signals?.time??0)/108);if(t.lastCycle===cycle||!s.beginCrossing(p,e))return;t.lastCycle=cycle;}
